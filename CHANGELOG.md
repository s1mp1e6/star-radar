...
- 通用 Webhook 通知通道（Server酱/钉钉/飞书/自建，任意接收 JSON POST 的服务）
- 三类事件：🔔 新 release / 🚀 大涨星（日涨幅阈值可配，默认 500）/ 📡 每日摘要（21:00 推送当日 Top5）
- 事件检测内置于追踪同步（每事件每仓库每日最多 1 次，单次同步最多 5 条防轰炸）
- 设置页通知卡片：开关/URL/事件勾选/阈值/一键测试/失败原因展示
- 通知适配器注册表（未来邮件/IM 零侵入）；通知失败只记录不阻塞同步
